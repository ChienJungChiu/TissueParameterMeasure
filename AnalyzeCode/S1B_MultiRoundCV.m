...
RTpixel = [206 225; 164 193; 127 155; 98 120; 64 83; 32 49];  %20250118
DeleteBG = 0;
Hbin = 8;
...
for SDS = 1:length(time)
    for RN = 1:round
        ...
        for image_num = 1:KineticSeriesLength
            ...
            image_temp(:,:,image_num)= imread(['ph1_f11_' num2str(time(SDS)) '_X' num2str(image_num) '.tif']);
            %% Do random track step
            spectra(image_num,:,SDS) = sum(image_temp(RTpixel(SDS,1):RTpixel(SDS,2),:,image_num),1);
            %mean_spectra{SDS}(RN,:) =  mean(spectra(image_num,:,SDS_num(SDS)),1);
        end
        mean_spectra{SDS}(RN,:) =  mean(spectra(:,:,time(SDS)),1);
        %plot_mean = mean_spectra(SDS,RN,:);
        subplot(2,3,SDS);
        plot(mean_spectra{SDS}(RN,:),'Color',color_arr(RN,:),'LineWidth',lineWidth);
